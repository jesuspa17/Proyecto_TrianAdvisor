...
public class MainActivity extends AppCompatActivity
    ...
    //AsyncTask que obtiene mis datos a partir del sessionToken dado.
    private class ObtenerMisDatosTask extends AsyncTask<String, Void, Usuario> {
        @Override
        protected Usuario doInBackground(String... params) {

            if (params != null) {
                Call<Usuario> usuarioCall = Servicio.instanciarServicio().obtenerMisDatos(params[0]);
    ...
    private class CerrarSesionTask extends AsyncTask<Void, Void, Void> {
        ...
        protected Void doInBackground(Void... params) {
            ...
            Call<Usuario> usuarioCall = Servicio.instanciarServicio().cerrarSesion(sessionToken);
            try {
                usuarioCall.execute();
            } catch (IOException e) {
                e.printStackTrace();
